...
def _find_world_xml() -> str:
    ...
    try:
        ...
        # pkg_share is <prefix>/share/mujoco_ros_utils — go up to <prefix>/share/xml/
        candidate = os.path.join(os.path.dirname(pkg_share), "xml", "test_scene_manager_world.xml")
        if os.path.isfile(candidate):
            return candidate
    except Exception:
        pass
...
def main():
    ...
    try:
        ...
        ready = False
        try:
            cli = _probe.create_client(_GBP, "/scene_manager/get_body_pose")
